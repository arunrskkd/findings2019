...
function acodez_themes_scripts() {
	...
	wp_register_script('owl', get_template_directory_uri() . '/js/owl.carousel.js', array('jquery'), '', true);
	wp_register_script('aos', get_template_directory_uri() . '/js/aos.js', array('jquery'), '', true);
	wp_register_script('tilt', get_template_directory_uri() . '/js/tilt.js', array('jquery'), '', true);
	wp_register_script('nextparticle', get_template_directory_uri() . '/js/nextparticle.js', array('jquery'), '', true);
...
function custom_post_type() {
	...
	$argsBrands = array(
		...
		'labels'                => $labelsBrands,
		'supports'              => array( 'title', 'editor','excerpt', 'thumbnail', 'custom-fields' ,'comments', 'post-templates'),
		'hierarchical'          => false,
		'public'                => true,
		'show_ui'               => true,
		'show_in_menu'          => true,
	...
	register_post_type( 'product', $argsproduct );
...
// Now register the taxonomy
  ...
  register_taxonomy('types',array('product'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
...
add_action('wp_ajax_nopriv_more_events', 'bi_more_events');
add_action('wp_ajax_more_events', 'bi_more_events');

function bi_more_events() {
...
function more_brands_ajax(){
    ...
    if ($loop -> have_posts()) :  while ($loop -> have_posts()) : $loop -> the_post();
	   $res.= ' <li> <div class="sectin">';
               ...
               if($imageurl){ 
					$res .=' <div class="imgsect">';
                    
					$res .= '<img src="'.$imageurl.'">';
                   $res .= '</div>';
                } 
				$res .='<div class="textset">';
				$res .='<h3>' . get_the_title() . '</h3> ';   
				$res .='</div> ';
				$res .=' </div></li>';
...
add_action('wp_ajax_nopriv_more_brands_ajax', 'more_brands_ajax'); 
add_action('wp_ajax_more_brands_ajax', 'more_brands_ajax');
